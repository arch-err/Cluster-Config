...
Pocket ID is the identity provider at `https://auth.apps.home`. Helm settings live in `kubernetes/platform/identity/pocket-id/values.yaml`; the configured existing claim is `pocket-id-local`. Its database contains identity/enrollment state that cannot be reconstructed from Git alone.
