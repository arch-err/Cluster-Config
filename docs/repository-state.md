...
| `storage-local-bulk` | Redundant: identical to the production baseline; its clean worktree can be detached before deleting the branch |
...
| `feat/n8n-infra-oauth` | Redundant after verifying its follow-up fix remains on `fix/n8n-syncwave-namespace`; feature merged through PR #5 |
