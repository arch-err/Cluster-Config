...
At the initial freeze, live inspection confirmed Grafana Git-sync and the three Homepage asset URLs still referenced `v2`. The earlier promotion changed those to `main` in desired state; the refactoring preparation restored their `v2` references to match deployed behavior. The approved September 14 migration below supersedes that temporary hold. Keep referenced files available at any branch/path fetched directly by running workloads; ArgoCD auto-sync settings do not suspend in-pod Git-sync processes.
...
## Grafana/Homepage source migration and v2 retirement (2026-09-14)

J approved finishing the move off `v2`, with backups and CI deferred to separate work. All three background assets and all four dashboard JSON files on `main` were byte-identical to the former `v2` files. Grafana's init container and sidecar now use `--ref=main`, and its provider points to `kubernetes/platform/observability/grafana/dashboards`. The three Homepage URLs point to each owner's reorganized asset directory on `main`.

Local renders and Argo comparisons showed only the source pointers/provider path and related configuration checksums. Scoped syncs at `1a0f9c5` applied one ConfigMap and one Deployment per Application, with Grafana chart `8.7.1` and Homepage chart `2.1.0`. No pruning or software upgrades were included. The original main archive remains `archive/main-2026-09-06`; the former v2 baseline is archived as `archive/v2-2026-09-14` before deleting its branch. The public-isolation PoC worktree and its uncommitted work remain preserved.

All four scoped syncs and rollouts succeeded and returned zero hard-refresh diff. Grafana retained the same four dashboard UIDs, titles and folders, with all mounted dashboard files matching the repository hashes. All Homepage frontends returned HTTP 200 and their configured image URLs returned the exact expected bytes. Storage specs/identities and Application specs were unchanged; the only workload-spec changes were the approved references and checksums. Auto-sync remains disabled everywhere. Current workload, pod and ConfigMap inspection found no remaining `v2` file references. The archive tag was verified remotely before deleting `v2`, and the PoC branch's obsolete upstream association was removed. Private verification records are under `~/.local/state/cluster-config/retire-v2/`. `just check` passed.
