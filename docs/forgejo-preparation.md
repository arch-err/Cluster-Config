...
SSH selects the destination by IP and port, not an HTTP Host header or TLS SNI. Consequently, any other DNS name resolving to this shared IP can also reach Forgejo on port 22. SSH authentication and Forgejo authorization still apply. Do not add a second DNS address expecting clients to pick one by protocol. No network changes have been applied.
